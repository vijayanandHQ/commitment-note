        ...
        <div class="card shadow-sm">
            <div class="card-header bg-white py-3 d-flex align-items-center justify-content-between">
                <h5 class="card-title mb-0 text-primary">
                    <i class="bx bx-bar-chart-alt-2 me-2"></i>CS Sales Report
                </h5>
            ...
            <div class="card-body">
                @if($commitments->count() > 0)
                    ...
                    <div class="table-responsive" id="mainScroll">
                        <table class="table table-bordered align-middle" id="salesTable">
                            ...
                            <tbody id="salesTableBody">
                                ...
                                @php
                                    $products      = $note->products; // already filtered: contacted_status=0
                                            ...
                                            <div class="d-flex flex-column gap-1">
                                                @foreach($products as $prod)
                                                    <div style="height:38px;" class="d-flex align-items-center justify-content-center">
                                                        <span class="badge bg-secondary">{{ $prod->order_qty ?? 0 }}</span>
                                                    </div>
                                                @endforeach
                                            </div>
                                        @endif
                                            ...
                                            <div class="d-flex flex-column gap-1">
                                                @foreach($products as $prod)
                                                    <div style="height:38px;" class="d-flex align-items-center">
                                                        <span class="small">{{ $prod->supplier->name ?? '-' }}</span>
                                                    </div>
                                                @endforeach
                                            </div>
                                        @endif
                                    ...
                                    <td class="text-start">
                                        @if($productCount > 0)
                                            <div class="d-flex flex-column gap-1">
                                                @foreach($products as $prod)
                                                    <div style="height:38px;" class="d-flex align-items-center">
                                                        <span class="small text-muted">{{ $prod->remarks ?? '-' }}</span>
                                                    </div>
                                                @endforeach
                                            </div>
                                        @endif
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-3">
                        <div class="text-muted small">
                            ...
                            Showing {{ $commitments->count() }} commitment(s) with contacted products &nbsp;|&nbsp;
                ...
                @else
                    <div class="alert alert-info text-center py-5">
                        ...
                        No records found with <strong>contacted_status = 0</strong>.
                    </div>
                @endif
            </div>
        </div>
...
#salesTable thead th:last-child { border-right: none !important; }
#salesTable thead .sticky-col {
...
#salesTable tbody td:last-child { border-right: none !important; }
#salesTable tbody tr:last-child td { border-bottom: none !important; }
#salesTable tbody tr.sales-row:hover td { background: #fafbff !important; }
#salesTable tbody tr.sales-row:hover .sticky-col { background: var(--brand-primary-lt) !important; }

#salesTable .sticky-col {
  position: sticky;
  left: 0;
  z-index: 2;
  background: #fff;
...
.badge-delivery-home {
  ...
  border:1px solid #c7d2fe !important; padding:6px 16px !important;
  border-radius:10px !important; text-transform:uppercase; font-weight:700; font-size:10px;
}
.badge-delivery-medical {
  background-color: #16A34A !important; color:#fff !important;
  border:1px solid #bbf7d0 !important; padding:6px 16px !important;
  border-radius:10px !important; text-transform:uppercase; font-weight:700; font-size:10px;
}
...
#salesTable .small.text-muted.fw-bold {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function sortRows(col, dir) {
        ...
        rows.sort((a, b) => {
            let va, vb;
            switch (col) {
                case 'name':       va = a.dataset.name;       vb = b.dataset.name;       return dir === 'asc' ? va.localeCompare(vb) : vb.localeCompare(va);
                case 'phone':      va = a.dataset.phone;      vb = b.dataset.phone;      return dir === 'asc' ? va.localeCompare(vb) : vb.localeCompare(va);
                case 'salesperson':va = a.dataset.salesperson;vb = b.dataset.salesperson;return dir === 'asc' ? va.localeCompare(vb) : vb.localeCompare(va);
                case 'type':       va = a.dataset.type;       vb = b.dataset.type;       return dir === 'asc' ? va.localeCompare(vb) : vb.localeCompare(va);
                case 'comments':   va = a.dataset.comments;   vb = b.dataset.comments;   return dir === 'asc' ? va.localeCompare(vb) : vb.localeCompare(va);
                case 'supplier':   va = a.dataset.supplier;   vb = b.dataset.supplier;   return dir === 'asc' ? va.localeCompare(vb) : vb.localeCompare(va);
                case 'remarks':    va = a.dataset.remarks;    vb = b.dataset.remarks;    return dir === 'asc' ? va.localeCompare(vb) : vb.localeCompare(va);
                ...
                case 'orderqty':   return dir === 'asc' ? (parseInt(a.dataset.orderqty)||0) - (parseInt(b.dataset.orderqty)||0) : (parseInt(b.dataset.orderqty)||0) - (parseInt(a.dataset.orderqty)||0);
                case 'sno':
        ...
        document.querySelectorAll('#salesTable .sortable').forEach(th => {
